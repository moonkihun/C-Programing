...
void change(char *s){ // 문자를 거꾸로 바꿔줌
    int lens1=strlen(s);
    char *s1=(char*)calloc(sizeof(char),lens1);
    int i;
    for(i=0;i<strlen(s);i++)
        s1[i] = s[strlen(s)-i-1];
    s1[i] ='\0';
...
void swap(char *p1,char *p2){// p1과 p2 자리 바꿈
    int lenp1=strlen(p1);
    char *tmp=(char*)calloc(sizeof(char),lenp1);
    strcpy(tmp,p1);
    strcpy(p1,p2);
    strcpy(p2,tmp);
}
int cal(char *s){ // 어떤 계산이 필요한지 1차적으로 제어하는 함수
    int lens=strlen(s);
    char *data=(char*)calloc(sizeof(char),lens);
    char *data_after=(char*)calloc(sizeof(char),lens);
    char *p1=(char*)calloc(sizeof(char),lens);
    char *p2=(char*)calloc(sizeof(char),lens);
...
{    int lenp1=strlen(p1);
    ...
    char *dap=(char*)calloc(sizeof(char),lenp1+lenp2+1);
    char *after=(char*)calloc(sizeof(char),lenp2+lenp2+1);
    if(p2[0] == '-'){ // p2가 음수인지 검사
        flag=-1; // 음수인것을 저장해놓음
        strcpy(p2,p2+1); // - 부호제거
    }
...
{ int lenp1=strlen(p1);
	int lenp2=strlen(p2);
    char *p1_point=(char*)calloc(sizeof(char),lenp1);
	char *p2_point=(char*)calloc(sizeof(char),lenp2);
    int i,k,min=0;
...
{   int lenp1=strlen(p1);
    ...
    char *p1_point=(char*)calloc(sizeof(char),lenp1);
    char *p2_point=(char*)calloc(sizeof(char),lenp2);
    char *tmp=(char*)calloc(sizeof(char),lenp1+2);
    char *tmp1=(char*)calloc(sizeof(char),lenp1+2);
    ...
    for (i = 0; i < cnt; i++) {
        ...
        if (nums[i].set == 1) {
            ...
            if (nums[i].n == ')') {
                ...
                stack[j] = 0;
                if (Ssp >= 1) {
                    if (stack[Ssp - 1] == '-' && ( stack[Ssp - 2] == '(' || stack[Ssp - 2] == '+' || stack[Ssp - 2] == '-' || stack[Ssp - 2] == '*')) {
                        num[Psp].n = -1;
                        num[Psp].set = 1;
                        Psp++;
                        num[Psp].n = '*';
